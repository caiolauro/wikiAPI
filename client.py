...
def postSampleDocuments():
    responses:list = [requests.post(URL, data={"title": document["title"], "content": document["content"]}) for document in SAMPLE_DOCUMENTS]
    return responses

def postOneSampleDocument():
    payload = {'title': 'example title', 'content': 'example content'}
    responses:str = requests.post(URL, data=payload)
    return responses

def deleteAllDocumentsFromCollection():
